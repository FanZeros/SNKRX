...
function Seeker:on_collision_enter(other, contact)
  ...
  if other:is(Wall) then
    ...
    local cur_player = main.current and main.current.player
    if cur_player and cur_player.heavy_impact then
      if self.being_pushed then
        self:hit(self.push_force, nil, nil, true)
      end
    end

    if cur_player and cur_player.tremor then
      if self.being_pushed then
        camera:shake(2, 0.5)
        earth1:play{pitch = random:float(0.95, 1.05), volume = 0.5}
        Area{group = main.current.effects, x = self.x, y = self.y, r = self.r, w = 0.75*self.push_force*(cur_player.area_size_m or 1), color = yellow[0], dmg = self.push_force/2, parent = cur_player}
      end
    end
...
function Seeker:curse(curse, duration, arg1, arg2, arg3)
  ...
  if main.current and main.current.player and main.current.player.whispers_of_doom then
  ...
  if main.current and main.current.player and main.current.player.hextouch then
...
function EnemyProjectile:on_trigger_enter(other, contact)
  ...
  elseif other:is(Critter) then
    if main.current and main.current.player and main.current.player.meat_shield then
      self:die(self.x, self.y, nil, random:int(2, 3))
      other:hit(1000)
    end
