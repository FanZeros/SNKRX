...
function Arena:on_enter(from, level, loop, units, passives, shop_level, shop_xp, lock)
  ...
  if self.level == 1000 then
    self.level_1000_text = Text2{group = self.ui, x = gw/2, y = gh/2, lines = {{text = '[fg, wavy_mid]蛇蛇小队', font = fat_font, alignment = 'center'}}}
  ...
  self.settings_button = Button{group = self.ui, x = 24, y = 13, force_update = true, button_text = '设置', fg_color = 'bg10', bg_color = 'bg', action = function(b)
...
function Arena:quit()
  ...
  if self.level % 25 == 0 then
      ...
      trigger:after(2.5, function()
        ...
        if current_new_game_plus == 6 then
          ...
          self.try_ng_text = Text2{group = self.ui, x = gw - 187, y = gh - 20, force_update = true, lines = {
            {text = '[bg10]新挑战(+难度,+1最大蛇长):', font = pixul_font},
          }}
          self.credits_button = Button{group = self.ui, x = gw - 40, y = gh - 68, force_update = true, button_text = '制作组', fg_color = 'bg10', bg_color = 'bg', action = function() self:create_credits() end}
        end
      end)
    ...
    if not self.arena_clear_text then self.arena_clear_text = Text2{group = self.ui, x = gw/2, y = gh/2 - 48, lines = {{text = '[wavy_mid, cbyc]战斗胜利!', font = fat_font, alignment = 'center'}}} end
    self:gain_gold()
  ...
  if self.boss_level then
    if self.start_time <= 0 then
      graphics.push(self.x2 - 106, self.y1 - 10, 0, self.hfx.condition2.x, self.hfx.condition2.x)
        graphics.print_centered('击败精英', fat_font, self.x2 - 106, self.y1 - 10, 0, 0.6, 0.6, nil, nil, fg[0])
      graphics.pop()
    end
  else
...
function Arena:create_credits()
  ...
  Text2{group = self.credits, x = 60, y = 80, lines = {{text = '[blue]引用库: ', font = pixul_font}}}
  ...
  Text2{group = self.credits, x = 70, y = 190, lines = {{text = '[red]测试员: ', font = pixul_font}}}
...
function Arena:transition()
  ...
  TransitionEffect{group = main.transitions, x = self.player.x, y = self.player.y, color = state.dark_transitions and bg[-2] or self.color, transition_action = function(t)
    ...
    {text = '[wavy_lower, ' .. tostring(state.dark_transitions and 'fg' or 'bg') .. ']获得金币: 0 + 0', font = pixul_font, alignment = 'center', height_multiplier = 1.5},
    {text = '[wavy_lower, ' .. tostring(state.dark_transitions and 'fg' or 'bg') .. ']利息: 0', font = pixul_font, alignment = 'center', height_multiplier = 1.5},
    {text = '[wavy_lower, ' .. tostring(state.dark_transitions and 'fg' or 'bg') .. ']总计: 0', font = pixul_font, alignment = 'center'}
  }, global_text_tags)}
end
